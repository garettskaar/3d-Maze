...
function pushHorizontalTexCoords(vertexTextureCoords, x, y, z){
...
function pushVerticalTexCoords(vertexTextureCoords, x, y, z){
    vertexTextureCoords.push(
        0.0, height,
        0.0, 0.0,
        width, 0.0,
...
function buildMaze(m) {
    ...
    for (var j = 0; j < m.x * 2 + 1; j++) {
        if (0 == j % 2) {
            for (var k = 0; k < m.y; k++) {
                if (k > 0 || j > 0) { // "-"
                    pushHorizontalVerticies(vertexPositions, k - h, 0, (j - 1) / 2 - h);
                    pushHorizontalTexCoords(vertexTextureCoords,  k - h, 0, (j - 1) / 2 - h);
                }
            }
        } else {
            for (var k = 0; k < m.y + 1; k++) {
                if (j != m.x * 2 - 1 || k != m.y) { // |
                    pushVerticalVerticies(vertexPositions, k - h, 0, (j - 2) / 2 - h);
                    pushVerticalTexCoords(vertexTextureCoords, k - h, 0, (j - 2) / 2 - h);
                }
            }
        }
    }
